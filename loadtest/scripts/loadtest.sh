...
function run_simapp() {
  export APP_BIN=$(which simd 2>/dev/null)
  export APP_HOME="$HOME/.simapp"
  export APP_DENOM="stake"
  export APP_CHAIN_ID="cosmos"
  ...
  for block_time in "block_timout_with_500ms"; do
    run $block_time
  done
}

function run_sei() {
  export APP_BIN=$(which seid 2>/dev/null)
  export APP_HOME="$HOME/.sei"
  export APP_DENOM="usei"
  export APP_CHAIN_ID="sei"
  export APP_ADMIN_KEY_NAME="admin"
  export APP_TEST_ACCOUNTS=1000
  export APP_TEST_ACCOUNTS_DIR="$HOME/test_accounts"
  if [ "$1" == "init" ]; then
    init_genesis && update_config
    exit 0
  fi
  for block_time in "block_timout_with_500ms"; do
    run $block_time
...
function run_dex() {
  export APP_BIN=$(which fxdexd 2>/dev/null)
  export APP_HOME="$HOME/.fxdex"
  export APP_DENOM="USDT"
  export APP_CHAIN_ID="dex-530-1"
  export APP_ADMIN_KEY_NAME="admin"
  export APP_TEST_ACCOUNTS=3000
  export APP_TEST_ACCOUNTS_DIR="$HOME/test_accounts"
  if [ "$1" == "init" ]; then
