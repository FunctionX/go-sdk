...
import (
	...
	"github.com/functionx/go-sdk/cosmos/crypto"
	"github.com/functionx/go-sdk/cosmos/crypto/ethsecp256k1"
	"github.com/functionx/go-sdk/cosmos/crypto/hd"
...
func NewAccountFromGenesis(genesisFilePath, keyDir string) (*Accounts, error) {
	...
	if !strings.HasPrefix(adminAddrStr, "0x") {
		adminAddr := cosmostypes.MustAccAddressFromBech32(adminAddrStr)
		prefix = adminAddr.GetPrefix()
	}
...
func CreateGenesisAccounts(addrPrefix string, number int, outDir string) error {
	...
	for i := 0; i < number; i++ {
		...
		var privBz []byte
		privBz, err = hd.Derive(mnemonic, "", bip44Params.String())
		if err != nil {
			return err
		}
		...
		if addrPrefix == "0x" {
			...
			address = cosmostypes.NewAccAddress(privKey.PubKey().Address(), addrPrefix).String()
		}
